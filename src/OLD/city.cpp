...
City::City(short righe, short colonne, float oneway_fraction, short gaussian_mean, short gaussian_sigma, short min_road_length, short max_road_length):_n_rows(righe), _n_coloumns(colonne)
{ 
    std::ofstream logFile("/home/simone/Scrivania/city-log.txt");

    logFile << "Inizio creazione della città\n";
    logFile.flush();

    Road::set_statistics(gaussian_mean, gaussian_sigma, min_road_length, max_road_length);

    logFile << "Statistiche impostate\n";
    logFile.flush();
    ...
    logFile << "Inizio primo for, rows = " << _n_rows << ", cols = " << _n_coloumns << "\n";
    logFile.flush();
    ...
    for( int i = 0; i < _n_rows*_n_coloumns; i++)
    {
        ...
        logFile << "Chi è il fucking bastard" << "\n";
        logFile.flush();
    ...
    logFile << "Inizio secondo for, rows = " << _n_rows << ", cols = " << _n_coloumns << "\n";
    logFile.flush();
