...
napi_value myMain(napi_env env, napi_callback_info info){
    ...
    double l1, l2, p, n_cars;

    status = napi_get_value_double(env, args[0], &l1);
    status = napi_get_value_double(env, args[1], &l2);
    status = napi_get_value_double(env, args[2], &p);
    status = napi_get_value_double(env, args[3], &n_cars); //TODO crea variabile

    Simulator dio = Simulator();
    cout << "Inizio a creare la città...\n";
    dio.create_city(l1,l2,p);
    ...
    while (false && dio.get_cars_at_destination() < N_CARS) //TODO sposta in metodo start
    {        
